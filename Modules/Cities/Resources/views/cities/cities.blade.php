                ...
                @foreach($cities as $key => $value)
                    <tr>
                      ...
                      <td>
                        ...
                        <form action="{{route('deletecities')}}" method="post" style="display: inline-block;">
                            {{csrf_field()}}
        ...
        <div class="modal-dialog">
          <div class="modal-content bg-primary">
            <div class="modal-header">
              <h4 class="modal-title">إضافة محافظة جديد</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body">
              <form action="{{route('storecities')}}" method="post">
                    {{csrf_field()}}
                    <div class="row">
                      <div class="col-sm-12">
                        <label>إسم  المحافظة</label> <span class="text-danger">*</span>
                        <input type="text" name="name" class="form-control" placeholder="إسم  المحافظة " required="" style="margin-bottom: 10px">
                      </div>
      ...
      <div class="modal fade" id="modal-update">
        <div class="modal-dialog">
          <div class="modal-content bg-info">
            <div class="modal-header">
              <h4 class="modal-title">تعديل محافظة : <span class="item_name"></span></h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body">
              <form action="{{route('updatecities')}}" method="post" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <div class="row">
                      <div class="col-sm-12">
                        <input type="hidden" name="edit_id" value="">
                        <label>إسم  المحافظة</label> <span class="text-danger">*</span>
                        <input type="text" name="edit_name" class="form-control" placeholder="إسم المحافظة " required="" style="margin-bottom: 10px">
                      </div>
                    </div>  
                    <label> الدول</label> <span class="text-danger">*</span>
...
<script type="text/javascript">
    ...
    $('.edit').on('click',function(){
        ...
        $("select[name='edit_country_id'] > option").each(function() {
        if($(this).val() == country)
        {
          $(this).attr("selected","")
        }
