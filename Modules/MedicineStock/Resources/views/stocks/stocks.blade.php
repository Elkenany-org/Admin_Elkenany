    ...
    <div class="row">
        <div class="col-sm-12">
          <div class="card card-primary card-outline">
            <div class="card-header">
              <h5 class="m-0" style="display: inline;"> بورصة ادوية  {{$section->name}} <i class="fas fa-exclamation-circle" style="cursor: pointer;color:#FFC107" data-toggle="modal" data-target="#modal-secondary"></i></h5>
              <a href="{{route('addstocksmedicine')}}" class="btn btn-primary" style="float: left;">
                    إضافة صنف لبورصة 
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-hover table-striped">
                ...
                @foreach($stocks as $key => $value)
                      ...
                      <tr>
                    <form action="{{route('updateMembermedicine')}}" method="post" id="form{{$value->id}}" style="display: inline-block;">
                    {{ csrf_field() }}
                      <input type="hidden" class="form-control" value="{{$value->id}}" name="id">  
                          <td>{{$key+1}}</td>
                         
                          <td>{{$value->Company->name}}</td>
                          <td>{{$value->MedicSubs->name}}</td>
                          <td>{{$value->Comname->name}}</td>
                          <td>
                          ...
                          <div class="dropdown" style="display:inline">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                              <a href="{{route('showmovementsmedicine',$value->id)}}" style="width:100%;text-align:center" class="dropdown-item " type="submit"> التحديثات</a>
                      ...
                      <td>
                          <form action="{{route('Deletemembermedicine')}}" method="post" style="display: inline-block;">
                            {{csrf_field()}}
        ...
        <div class="modal fade" id="modal-secondary">
          ...
          <div class="modal-content bg-secondary">
            ...
            <p>هذه الصفحة خاصة ببورصة الادوية</p>
            </div>
          </div>
          </div>
        </div>
...
$(".updates").on('click',function(e){
  ...
  jQuery.ajaxSetup({
    ...
    $.ajax(
    {
        url: "{{route('updateMembermedicine')}}",
        type: 'post',
