...
<div class="card-body">
	<div class="row">
		<div class="col-sm-12">
			...
			<h6 class="m-0" style="display: inline;">تعديل مناقصة <span class="text-primary"> {{$tenders->title}} </span></h6>
			</div>
			<div class="card-body">
				<form action="{{route('Updatetenders')}}" method="post" enctype="multipart/form-data" novalidate>
					<div class="row">
						{{csrf_field()}}
						...
						<div class="row">
							...
							<div class="col-sm-8 offset-2" style="margin-top: 10px">
								<div class="from-group">
									<label class="text-primary">عنوان مناقصة: <span class="text-danger">*</span></label>
									<input type="text" name="title" class="form-control" value="{{$tenders->title}}" placeholder="عنوان مناقصة" required="">
								</div>
							</div>
                    ...
                    <div class="col-sm-6" style="margin-top: 10px">
                      <div class="from-group">
                        ...
                        @foreach($sections as $value)
                          <option value="{{$value->id}}"{{ isset($tenders) && $tenders->section_id == $value->id ? 'selected'  :'' }}>{{$value->name}}</option>
                        @endforeach
                        </select>
                      </div>
                    </div>
                    ...
                    <div class="col-sm-6" style="margin-top: 10px">
                      <div class="from-group">
                        ...
                        @foreach($cities as $value)
                          <option value="{{$value->id}}"{{ isset($tenders) && $tenders->city_id == $value->id ? 'selected'  :'' }}>{{$value->name}}</option>
                        @endforeach
                        </select>
                      </div>
                    </div>
                            ...
                            <div class="col-sm-4 offset-2" style="margin-top: 10px">
                                <div class="from-group">
                                    ...
                                    <input type="date" name="open_date" class="form-control" value="{{$tenders->open_date}}"  required="">
                                </div>
                            </div>
							...
							<div class="col-sm-6 offset-3 marbo" style="margin-top: 10px">
								...
								<img src="{{asset('uploads/tenders/avatar/'.$tenders->image)}}" onclick="ChooseAvatar()" id="avatar">
							</div>
						</div>
					</div>
					<div class="col-sm-12">
						<div class="row">
							...
							<div class="col-sm-12">
								...
								<textarea class="form-control"  id="full-featured-non-premium" rows="10" name="desc" value="{{$tenders->desc}}" placeholder="التفاصيل" required>{{$tenders->desc}}</textarea>
							</div>
						</div>
					</div>
						{{-- submit --}}
