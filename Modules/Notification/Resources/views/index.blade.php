                    ...
                    <table id="example1" class="table table-bordered table-hover table-striped">
                        ...
                        @foreach($notifications as $key => $value)
                            <tr>
                                ...
                                <td>{{$value->title}}<br>{{$value->body}}</td>
                                <td>{{$value->status == 0 ? 'مجدول' : 'تم الأرسال'}}</td>
                                <td>@if($value->image != "")<img src="{{$value->image_url}}" width="50px" height="50px">@else <img src="https://img.icons8.com/ios/50/000000/image.png"/> @endif</td>
                                <td>{{$value->date_at .' / '. $value->time_at}}</td>
                                <td>{{$value->company != null ? $value->company->name : ''}}</td>
                                <td>
                                    @if($value->status == 0)
                            ...
                            <div class="modal fade" id="exampleModalDelete{{$value->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <form action="{{route('delete_notification',$value->id)}}" method="GET" >
                                        @csrf
