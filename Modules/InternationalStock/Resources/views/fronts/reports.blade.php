...
<title>   التقارير الاسبوعية</title>
<style type="text/css">
...
<!-- Start Header -->
  ...
  <div class="breadcrumb__container nt-0 container">
        <div class="custom__breadcrumb">
            <h1 class="page__title"> التقارير الاسبوعية</h1>
        </div>
    </div>
    <!-- End breadcrumb -->
        ...
        <form class="box-tabs">
            ...
            <section class="tabs tabs-1">
                ...
                <select name="sort" onChange="window.location.href=this.value">
                    <option value="{{ route('front_reports') }}" {{ isset($sort) && $sort == "0" ? 'selected'  :'' }}> الابجدي</option>
                    <option value="{{ route('front_reports_view') }}"{{ isset($sort) && $sort == "1" ? 'selected'  :'' }}> َالاكثر تداولا</option>
