		...
		<div class="col-sm-12">
          <div class="card card-primary card-outline">
		    <div class="card-header">
				<h6 class="m-0" style="display: inline;">تعديل حركة<span class="text-primary"> {{$ships->name}} </span></h6>
			</div>
            <div class="card-body">
            	<form action="{{route('Updateships')}}" method="post" enctype="multipart/form-data">
					{{csrf_field()}}
					<input type="hidden" name="id" value="{{$ships->id}}">
					<div class="row">
								...
								<div class="col-sm-3"style="margin-top: 20px;">
									<div class="from-group">
										...
										<input type="text" name="name" class="form-control" value="{{$ships->name}}" placeholder=" اسم السفينة" required="">
									</div>
								</div>
								{{-- load --}}
								<div class="col-sm-3"style="margin-top: 20px;">
									<div class="from-group">
										...
										<input type="number" name="load" class="form-control" value="{{$ships->load}}" placeholder=" حمولة السفينة" required="">
									</div>
								</div>
								{{-- country --}}
								<div class="col-sm-3"style="margin-top: 20px;">
									<div class="from-group">
										...
										<input type="text" name="country" class="form-control" value="{{$ships->country}}" placeholder="  المنشأ" required="">
									</div>
								</div>
								{{-- date --}}
								<div class="col-sm-3"style="margin-top: 20px;">
									<div class="from-group">
										...
										<input type="date" name="date" class="form-control" value="{{$ships->date}}" placeholder="  التاريخ" required="">
									</div>
								</div>
								{{-- agent --}}
								<div class="col-sm-3"style="margin-top: 20px;">
									<div class="from-group">
										...
										<input type="text" name="agent" class="form-control" value="{{$ships->agent}}"" placeholder="   الوكيل الملاحي" required="">
									</div>
								</div>
								{{-- dir_date --}}
								<div class="col-sm-3"style="margin-top: 20px;">
									<div class="from-group">
										...
										<input type="date" name="dir_date" class="form-control" value="{{$ships->dir_date}}"" placeholder="    تاريخ التوجيه" required="">
									</div>
								</div>
								{{-- ports --}}
								<div class="col-sm-4"style="margin-top: 20px;">
									<div class="from-group">
										...
										@foreach($ports as $value)
											<option value="{{$value->id}}"{{ isset($ships) && $ships->port_id == $value->id ? 'selected'  :'' }}>{{$value->name}}</option>
										@endforeach
										</select>
									</div>
								</div>
								...
								<div class="col-sm-4"style="margin-top: 20px;">
									<div class="from-group">
										...
										@foreach($products as $value)
											<option value="{{$value->id}}"{{ isset($ships) && $ships->product_id == $value->id ? 'selected'  :'' }}>{{$value->name}}</option>
										@endforeach
										</select>
									</div>
								</div>
								...
								<div class="col-sm-12 " style="margin-top:20px">
									...
									<div class="col-sm-3 result_style">
										<input type="radio" value="{{$ships->Company->id}}"  class="company_id" name="company_id" checked>
										<label class="text-info">{{$ships->Company->name}}</label>
									</div>
									</div>
								</div>
			...
			<div class="modal-content bg-secondary">
				...
				<p>هذه الصفحة خاصة    بتعديل حركة سفن</p>
				</div>
			</div>
			</div>
			</div>
